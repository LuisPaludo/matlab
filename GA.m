classdef GA
    ...
    methods
        ...
        function obj = GA(popSize, numGenerations, crossoverRate, mutationRate, paramNum, paramBounds, func, Rs, Rr, Lm)
            load('Dados/Iqs.mat', 'd_iqs_vetor');
            load('Dados/Ids.mat', 'd_ids_vetor');
            load('Dados/wr.mat', 'd_wr_vetor');
            obj.popSize = popSize;
            ...
            obj.generation = 0;
            obj.real_ids_vetor = d_ids_vetor;
            obj.real_iqs_vetor = d_iqs_vetor;
            obj.real_wr_vetor = d_wr_vetor;
            obj.simp_wr = 891.11;
            obj.simp_iqs = 4.85;
            obj.simp_ids = 9.72;            
            
            % Define valores padrão para Rs, Rr e Lm se eles não forem fornecidos
            if nargin < 8
                Rs = 0;
            end
            if nargin < 9
            ...
            if strcmp(func, 'f_motor_high_data')
                obj.CostFunction = @obj.f_motor_high_data;
            end
            if strcmp(func, 'f_motor_low_data')
                obj.CostFunction = @obj.f_motor_low_data;
            end
            if strcmp(func, 'f_motor_high_data_complement')
                obj.CostFunction = @obj.f_motor_high_data_complement;
            end
        end
        ...
        function [cost] = f_motor_low_data(obj, x)
            ...
            for k = 1:Np
                ...
                for ksuper=1:p
                    ...
                    wrpm = wr*2/Polos*60/(2*pi);                             % Velocidade rotórica em rpm

                end

            end

            Objetivo.Ids = obj.simp_ids;                           % Valor desejado de Iqs
            Objetivo.Iqs = obj.simp_iqs;                           % Valor desejado de Ids
            Objetivo.wr = obj.simp_wr;                          % Valor desejado de wr

            erro_Iqs = abs(Objetivo.Iqs - Iqs);
            erro_Ids = abs(Objetivo.Ids - Ids);
            erro_wr = abs(Objetivo.wr - wrpm);

            % cost = erro_Iqs^2 + erro_Ids^2 + erro_wr^2;

            cost = cost + Tsc * (erro_Iqs + erro_Ids + erro_wr);

        end

        function [cost] = f_motor_high_data(obj, x)
            ...
            for k = 1:Np
                ...
                w_Iqs = 2; % Peso para erro_Iqs
                w_Ids = 2; % Peso para erro_Ids
                w_wr = 0;  % Peso para erro_wr (mantido em 1 se você quiser dar menos importância)

                % Cálculo do erro
                erro_Iqs = abs(obj.real_iqs_vetor(k) - Iqs);
                erro_Ids = abs(obj.real_ids_vetor(k) - Ids);
                erro_wr = abs(obj.real_wr_vetor(k) - wrpm);

                % Aplicando os pesos no cálculo do erro
                cost = cost + Tsc*(w_Iqs * erro_Iqs^2 + w_Ids * erro_Ids^2 + w_wr * erro_wr^2);

            end

        end

        function [cost] = f_motor_high_data_complement(obj, x)
            ...
            Xm = weles*obj.Lm_solution;                     % Reatância de magnetização
            Xls = weles*Lls;                   % Reatância de dispersão do estator
            Xlr = weles*Llr;                   % Reatância de dispersão do rotor
            Xml = 1/(1/Xm + 1/Xls + 1/Xlr);
            ...
            for k = 1:Np
                ...
                for ksuper=1:p
                    ...
                    Fqs = Fqs + h*weles*(Vq - w/weles*Fds - obj.Rs_solution/Xls*(Fqs-Fqm));
                    Fds = Fds + h*weles*(Vd + w/weles*Fqs - obj.Rs_solution/Xls*(Fds-Fdm));

                    Fqr = Fqr - h*weles*((w-wr)*Fdr/weles + obj.Rr_solution/Xlr*(Fqr-Fqm));
                    Fdr = Fdr - h*weles*((wr-w)*Fqr/weles + obj.Rr_solution/Xlr*(Fdr-Fdm));
